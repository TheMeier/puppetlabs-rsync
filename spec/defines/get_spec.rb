...
describe 'rsync::get', type: :define do
  ...
  describe 'when using default class paramaters' do
    ...
    it {
      is_expected.to contain_exec('rsync foobar').with('command' => 'rsync -q -a example.com foobar',
  ...
  describe 'when setting a user but not a keyfile' do
    ...
    it {
      is_expected.to contain_exec('rsync foobar')
        .with('command' => 'rsync -q -a -e \'ssh -i /home/mr_baz/.ssh/id_rsa -l mr_baz\' mr_baz@example.com foobar',
  ...
  describe 'when setting a user and a keyfile' do
    ...
    it {
      is_expected.to contain_exec('rsync foobar').with('command' => 'rsync -q -a -e \'ssh -i /path/to/keyfile -l mr_baz\' mr_baz@example.com foobar',
  ...
  describe 'when changing rsync options' do
    ...
    it {
      is_expected.to contain_exec('rsync foobar').with('command' => 'rsync -q -rlpcgoD example.com foobar',
  ...
  describe 'when setting a custom path' do
    ...
    it {
      is_expected.to contain_exec('rsync foobar').with('command' => 'rsync -q -a example.com barfoo',
